# Ja no Caminho Mobile (Android + iOS)
...
- `mobile/ios/`: native iOS project generated by Capacitor.
...
The iOS project requires macOS, Xcode, and CocoaPods. The checked-in project targets iOS 15+, which is the minimum supported by the Xcode SDK installed in the development environment.
...
For the quickest edit/build/install loop, boot the desired Simulator or connect an iPhone and run from the repository root:

```bash
npm --prefix mobile run ios:deploy
```

If more than one target is available, list them and pass the selected identifier:

```bash
npm --prefix mobile run ios:devices
npm --prefix mobile run ios:deploy -- --target DEVICE_ID
```

Open `mobile/ios/App/App.xcworkspace` in Xcode, select the `App` scheme, choose an iOS Simulator or a connected iPhone, and run. Always open the `.xcworkspace`, not the `.xcodeproj`, so the Capacitor CocoaPods are included.
...
Before running on a physical device or distributing through TestFlight/App Store, configure an Apple Developer team, signing, bundle identifier, and push-notification capability in Xcode. Push notifications also need APNs/Firebase iOS credentials; the current Android-specific printer and custom FCM native services have no iOS implementation yet.

## Notes

- This setup is ready for iterative Android work while web continues normally.
- The iOS shell uses the same hosted Hub architecture as Android and is now generated and synced from Capacitor.
