...
export type MercadoPagoConnectedAccountOwnerType = 'STORE' | 'MOTOBOY';

export type MercadoPagoConnectedAccountState = {
  ownerType: MercadoPagoConnectedAccountOwnerType;
  ownerId: string;
  returnTo?: string | null;
  createdAt?: number;
};

export class MercadoPagoConnectedAccountService {
  private log = logger.child({ scope: 'MercadoPagoConnectedAccountService' });
  ...
  detectCredentialMode(accessToken: string) {
  ...
  async validateCapabilities(accessToken: string) {
  ...
  signState(payload: MercadoPagoConnectedAccountState) {
    const normalized: MercadoPagoConnectedAccountState = {
      ownerType: payload.ownerType,
      ownerId: String(payload.ownerId || '').trim(),
      returnTo: String(payload.returnTo || '').trim() || null,
  ...
  parseState(state: string) {
    ...
    const parsed = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8')) as Record<string, any>;
    const ownerTypeRaw = String(parsed?.ownerType || '').trim().toUpperCase();
    const ownerType =
      ownerTypeRaw === 'MOTOBOY'
        ? 'MOTOBOY'
        : ownerTypeRaw === 'STORE'
          ? 'STORE'
          : parsed?.storeId
            ? 'STORE'
            : null;
    const ownerId = String(parsed?.ownerId || parsed?.storeId || '').trim();
    ...
    return {
      ownerType,
      ownerId,
      returnTo: String(parsed?.returnTo || '').trim() || null,
      createdAt: Number(parsed?.createdAt || 0) || undefined,
    } satisfies MercadoPagoConnectedAccountState;
  }

  createConnectUrl(payload: MercadoPagoConnectedAccountState) {
