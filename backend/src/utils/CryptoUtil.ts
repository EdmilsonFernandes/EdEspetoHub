import crypto from 'crypto';
import { env } from '../config/env';
import { Provide } from '../ioc/ioc';
import { Tokens } from '../ioc/injectiontokens';

type OrderAccessPayload = {
  oid: string;
  exp: number;
};

@Provide(Tokens.Utils.CryptoUtil)
export class CryptoUtil {
  private readonly TOKEN_TTL_MS = 30 * 24 * 60 * 60 * 1000;

  private base64UrlEncode(input: string): string {
    return Buffer.from(input)
      .toString('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=+$/g, '');
  }

  private base64UrlDecode(input: string): string {
    const normalized = input.replace(/-/g, '+').replace(/_/g, '/');
    const padding = normalized.length % 4 ? '='.repeat(4 - (normalized.length % 4)) : '';
    return Buffer.from(`${normalized}${padding}`, 'base64').toString('utf8');
  }

  private sign(payloadB64: string): string {
    return crypto
      .createHmac('sha256', `${env.jwtSecret}:order-access`)
      .update(payloadB64)
      .digest('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=+$/g, '');
  }

  public createOrderAccessToken(orderId: string): string {
    const payload: OrderAccessPayload = {
      oid: String(orderId || ''),
      exp: Date.now() + this.TOKEN_TTL_MS,
    };
    const payloadB64 = this.base64UrlEncode(JSON.stringify(payload));
    return `${payloadB64}.${this.sign(payloadB64)}`;
  }

  public verifyOrderAccessToken(token: string, orderId: string): boolean {
    if (!token || !orderId) return false;
    const [payloadB64, providedSig] = String(token).split('.');
    if (!payloadB64 || !providedSig) return false;
    const expectedSig = this.sign(payloadB64);
    if (providedSig !== expectedSig) return false;
    try {
      const payload = JSON.parse(this.base64UrlDecode(payloadB64)) as OrderAccessPayload;
      if (!payload?.oid || payload.oid !== String(orderId)) return false;
      if (!payload?.exp || Number(payload.exp) < Date.now()) return false;
      return true;
    } catch {
      return false;
    }
  }
}
