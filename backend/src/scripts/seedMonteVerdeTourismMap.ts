 ...
 *
 * @file: seedMonteVerdeTourismMap.ts
 * @Date: 2026-06-02
 */
...
import {
  ...
  MONTE_VERDE_TOURISM_SOURCE,
  MonteVerdeHospitalitySeed,
  MonteVerdeListingSeed,
} from '../utils/monteVerdeTourismSeedData';

const log = logger.child({ scope: 'seedMonteVerdeTourismMap' });

const destinationDescription = `${MONTE_VERDE_DESTINATION_SEED.description} Dados factuais importados como curadoria inicial; parcerias oficiais devem ser confirmadas pelos responsaveis.`;

const placeDescription = (seed: MonteVerdeHospitalitySeed) => {
  const typeLabel =
    seed.type === 'CHALE'
      ? 'Chale'
      : seed.type === 'HOTEL'
  ...
  return `${typeLabel} em Monte Verde cadastrado como curadoria inicial do Ja no Caminho. Fotos, tarifas e regras devem ser confirmadas pelo responsavel.`;
};

const listingDescription = (seed: MonteVerdeListingSeed) => {
  if (seed.category === 'RESTAURANTE_VISITAR') {
    return 'Lugar para comer ou visitar em Monte Verde. Confirme horarios, disponibilidade, entrega e valores diretamente com o estabelecimento.';
  }
  if (seed.category === 'PASSEIO') {
    return 'Experiencia local para visitantes em Monte Verde. Confirme disponibilidade, valores e requisitos diretamente com o responsavel.';
  }
  if (seed.category === 'ATRATIVO') {
    return 'Ponto de interesse da curadoria turistica de Monte Verde.';
  }
  if (seed.category === 'NOITE') {
    return 'Opcao local para sair ou conhecer a noite em Monte Verde. Confirme horarios antes de visitar.';
  }
  if (seed.category === 'LOJA') {
    return 'Compra local e produto regional em Monte Verde. Confirme horarios, disponibilidade e valores diretamente com o responsavel.';
  }
  return 'Registro de curadoria local. Confirme horarios, disponibilidade e valores diretamente com o responsavel.';
};

const getCtaType = (seed: MonteVerdeListingSeed) => {
...
const getCtaUrl = (seed: MonteVerdeListingSeed) => seed.whatsapp || seed.websiteUrl || seed.instagramUrl || null;
...
const seedDestination = async () => {
  const rows: Array<{ id: string }> = await AppDataSource.query(
    ...
    `,
    [
      MONTE_VERDE_DESTINATION_SEED.name,
      MONTE_VERDE_DESTINATION_SEED.slug,
      MONTE_VERDE_DESTINATION_SEED.city,
      MONTE_VERDE_DESTINATION_SEED.state,
      destinationDescription,
      MONTE_VERDE_DESTINATION_SEED.heroTitle,
      MONTE_VERDE_DESTINATION_SEED.heroSubtitle,
      MONTE_VERDE_DESTINATION_SEED.lat,
      MONTE_VERDE_DESTINATION_SEED.lng,
      MONTE_VERDE_DESTINATION_SEED.sortOrder,
    ]
  );
...
const seedHospitalityPlaces = async (destinationId: string) => {
  ...
  for (const seed of MONTE_VERDE_HOSPITALITY_SEEDS) {
...
const upsertListing = async (destinationId: string, seed: MonteVerdeListingSeed) => {
  const rows: Array<{ id: string }> = await AppDataSource.query(
    `
      WITH existing AS (
        SELECT id
...
const seedListings = async (destinationId: string) => {
  ...
  for (const seed of MONTE_VERDE_LISTING_SEEDS) {
...
const seed = async () => {
  ...
  if (!destinationId) {
    throw new Error('Nao foi possivel criar ou localizar o destino Monte Verde.');
  }

  const placesCount = await seedHospitalityPlaces(destinationId);
  const listingsCount = await seedListings(destinationId);

  log.info('Monte Verde tourism seed saved', {
    source: MONTE_VERDE_TOURISM_SOURCE,
    destinationId,
    placesCount,
    listingsCount,
  });
...
seed().catch(async (error) => {
  log.error('Failed to seed Monte Verde tourism map', { error });
