...
| `destinations.test.ts` | Destinos/chalés/serviços: criação de destino real, hospedagem, listing, vínculo com loja, conflito de slug, destinos inativos no admin, solicitações de parceria, aprovação/link de loja, claim de perfil existente sem duplicar, reenvio de convite e portal do parceiro com ativação, permissão e edição segura. |
