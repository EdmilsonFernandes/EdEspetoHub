...
export function AuthLayout({
  ...
  const spacingClass = showHeader
    ? 'pb-[calc(env(safe-area-inset-bottom)+0.85rem)] pt-[calc(env(safe-area-inset-top)+4.05rem)] sm:pb-10 sm:pt-[calc(env(safe-area-inset-top)+6rem)]'
    : 'py-3 sm:py-10';
  ...
  return (
    <div className={`min-h-[100dvh] overflow-x-clip bg-[#EEF2F7] px-4 ${spacingClass} relative ${isNativePlatform ? 'ds-native-nav-content' : ''}`}>
      ...
      <div className={`mx-auto grid w-full max-w-7xl ${shellMinHeightClass} items-stretch gap-4 sm:gap-6 lg:gap-7 lg:grid-cols-[1.06fr_minmax(520px,1fr)] relative z-10`}>
