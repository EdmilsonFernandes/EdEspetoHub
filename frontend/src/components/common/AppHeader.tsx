...
const toneClassByAction: Record<string, string> = {
...
export function AppHeader({
  ...
  return (
    <header className={`relative overflow-hidden rounded-2xl border ${variantShell[variant]}`}>
      ...
      <div className={`relative px-3 sm:px-4 lg:px-5 ${variant === 'operations' ? 'py-3 sm:py-3.5' : 'py-3 sm:py-4'}`}>
              ...
              action.href ? (
                <a
                  ...
                  className={`inline-flex min-h-[44px] items-center gap-1.5 rounded-xl border px-3 text-xs font-semibold transition ${
                    toneClassByAction[action.tone || 'ghost']
                  }`}
                >
                  {action.icon}
                  {action.label}
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-[200]">
          ...
          <aside className="absolute inset-x-0 bottom-0 rounded-t-3xl border border-slate-200 bg-white max-h-[76vh] overflow-y-auto">
