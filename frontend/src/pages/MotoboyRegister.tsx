...
export function MotoboyRegister() {
  ...
  return (
    <div className="min-h-screen motoboy-bg px-4 py-6 sm:py-10 space-y-5 overflow-x-hidden">
      <div className="max-w-lg mx-auto space-y-5">
        <MotoboyHeader title="Cadastro" subtitle="Crie sua conta e receba solicitações das lojas." />
        <div className="premium-card-glass p-5 sm:p-6 space-y-4 motoboy-fade-up">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-slate-500">Área do Entregador</p>
              <h2 className="text-xl font-black text-slate-900">Cadastro rápido e seguro</h2>
              <p className="text-sm text-slate-600 mt-1">Use um e-mail diferente do cadastro de lojista.</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 px-3 py-2.5">
              <div className="h-16 w-16 rounded-2xl border border-slate-200 bg-white overflow-hidden grid place-items-center text-slate-500 font-black shadow-sm shrink-0">
                {profilePreview ? (
              ...
              <label className="text-xs text-slate-600 font-semibold min-w-0">
                Foto do perfil (opcional)
                <input
                  type="file"
                  accept="image/*"
                  onChange={(event) => handleProfileUpload(event.target.files?.[0])}
                  className="mt-1 block w-full text-[11px] text-slate-500 file:mr-2 file:rounded-lg file:border-0 file:bg-slate-900 file:px-2.5 file:py-1.5 file:text-[11px] file:font-semibold file:text-white"
                />
              </label>
            </div>
            ...
            <div className="grid grid-cols-[110px_1fr] gap-2">
              <select
                ...
                className="input-premium w-full px-3 py-2.5 text-sm font-semibold"
              >
              ...
              <input
                ...
                className="input-premium w-full px-3 py-2.5 text-sm disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"
              />
            </div>
            <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-press w-full rounded-xl bg-[linear-gradient(120deg,var(--color-primary),color-mix(in_srgb,var(--color-primary)_60%,#f59e0b))] px-4 py-3 text-sm font-extrabold text-white shadow-[0_22px_48px_-32px_rgba(239,68,68,0.85)] disabled:opacity-50 disabled:cursor-not-allowed"
            >
